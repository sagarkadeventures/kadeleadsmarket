...
const FALLBACK_REDIRECT_URL = "https://afflat3d3.com/trk/lnk/786BE43A-66BF-4957-B2D1-CEF4DF250208/?o=15451&c=918273&a=516670&k=340953338760B4DF749BD4BFBB0C1B83&l=17035&s1=radcredapplynow";
...
function EnhancedLeadForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.status === 'validation_error' || result.status === 'error') {
        if (result.errors && Array.isArray(result.errors)) {
          ...
          result.errors.forEach(err => {
            if (!err || !err.Field) {
              console.warn('⚠️ Error without Field:', err);
              return;
            }
            
            const field = err.Field;
            const description = err.Description || 'Invalid value';
            
            if (field === 'PhoneHome' || field === 'phone') {
              newErrors.phone = description;
            } else if (field === 'ZipCode' || field === 'zip') {
              newErrors.zip = description;
            } else if (field === 'SSN' || field === 'ssn') {
              newErrors.ssn = description;
            } else if (field === 'BankABA') {
              console.error('❌ Banking error:', description);
              ...
              try {
                ...
                newErrors[fieldName] = description;
              } catch (e) {
                console.error('Could not process field:', field);
              }
            }
          });
      ...
      // ✅ REJECTED or DUPLICATE - Show success popup, then redirect to fallback
      else if (result.status === "rejected" || result.status === "duplicate") {
  ...
  // Render success state (for both sold and rejected/duplicate)
